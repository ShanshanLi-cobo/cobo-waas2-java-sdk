...
public class PayoutFeeData extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(PayoutFeeData.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!PayoutFeeData.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'PayoutFeeData' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<PayoutFeeData>() {
                @Override
                public void write(JsonWriter out, PayoutFeeData value) throws IOException {
                ...
                @Override
                public PayoutFeeData read(JsonReader in) throws IOException {
                    ...
                    PayoutFeeData newPayoutFeeData = new PayoutFeeData();
                    if (jsonObject.get("fee_type") == null) {
                        log.log(Level.WARNING, "Failed to lookup discriminator value for PayoutFeeData as `fee_type` was not found in the payload or the payload is empty.");
                    } else  {
                        ...
                        switch (jsonObject.get("fee_type").getAsString()) {
                            case "EVM_EIP_1559":
                                ...
                                newPayoutFeeData.setActualInstance(deserialized);
                                return newPayoutFeeData;
                            case "EVM_Legacy":
                            ...
                            case "TransactionRequestUtxoFee":
                                ...
                                return newPayoutFeeData;
                            default:
                                log.log(Level.WARNING, String.format("Failed to lookup discriminator value `%s` for PayoutFeeData. Possible values: EVM_EIP_1559 EVM_Legacy Fixed UTXO TransactionRequestEvmEip1559Fee TransactionRequestEvmLegacyFee TransactionRequestFixedFee TransactionRequestUtxoFee", jsonObject.get("fee_type").getAsString()));
                        }
                    }
                    ...
                    if (match == 1) {
                        PayoutFeeData ret = new PayoutFeeData();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for PayoutFeeData: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public PayoutFeeData() {
        super("oneOf", Boolean.FALSE);
    }

    public PayoutFeeData(TransactionRequestEvmEip1559Fee o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }

    public PayoutFeeData(TransactionRequestEvmLegacyFee o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }

    public PayoutFeeData(TransactionRequestFixedFee o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }

    public PayoutFeeData(TransactionRequestUtxoFee o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to PayoutFeeData
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            // throw new IOException(String.format("The JSON string is invalid for PayoutFeeData with oneOf schemas: TransactionRequestEvmEip1559Fee, TransactionRequestEvmLegacyFee, TransactionRequestFixedFee, TransactionRequestUtxoFee. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of PayoutFeeData given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of PayoutFeeData
     * @throws IOException if the JSON string is invalid with respect to PayoutFeeData
     */
    public static PayoutFeeData fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, PayoutFeeData.class);
    }

    /**
     * Convert an instance of PayoutFeeData to an JSON string
     *
     * @return JSON string
     */
